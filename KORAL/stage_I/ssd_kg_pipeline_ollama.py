...
# --------- CONFIG ---------
DEFAULT_MODEL = os.getenv("KG_LLM_MODEL", "llama3.1:8b")
...
class LLMBackend:
    def __init__(self, model: str = DEFAULT_MODEL):
        if OpenAI is None:
            raise RuntimeError(
                "OpenAI client not available. Install `openai`>=1.0"
            )
    ...
    def run(self, full_paper_text: str, taxonomy_json: Dict[str, Any], max_retries: int = 5) -> Dict[str, Any]:
        ...
        messages = [{"role": "system", "content": sys_prompt + "\n\nYou ONLY output JSON. Never output explanations."}]
        if addenda.strip():
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                log.info("----- LLM RAW OUTPUT START -----")
                log.info(content[:2000])
                log.info("----- LLM RAW OUTPUT END -----")
                """import re
...
def merge_chunk_results(results):
    ...
    for r in results:
        ...
        for t in r.get("triples", []):
            key = (t.get("s"), t.get("p"), str(t.get("o")))
            if key not in seen_triples:
                merged["triples"].append(t)
                seen_triples.add(key)

        # Others (just extend)
        merged["axioms"].extend(r.get("axioms", []))
        merged["mappings"].extend(r.get("mappings", []))
        merged["new_concepts"].extend(r.get("new_concepts", []))

    return merged

def reassign_entity_ids(result):
    id_map = {}
    new_entities = []
    ...
    for t in result["triples"]:
        if t["s"] in id_map:
            t["s"] = id_map[t["s"]]
        if isinstance(t["o"], str) and t["o"] in id_map:
            t["o"] = id_map[t["o"]]
...
def process_paper(
    ...
    result = coerce_json(merged_result)
    # ===== END CHUNKING =====

    # Rest stays the same...
    result["paper_id"] = result.get("paper_id") or make_paper_id(paper_path)
    result["paper_original_filename"] = paper_path.name
    safe_stem = slugify_filename(paper_path.stem)
